...
export function playerNickname(userId: string): string {
  ...
  if (fromClub) return fromClub.nickname;
...
export function ratingPlaceForId(playerId: string): number | undefined {
  ...
  if (poolIdx >= 0) return poolIdx + 1;

  const club = [...getClubDirectory()].sort((a, b) => a.nickname.localeCompare(b.nickname));
  const clubIdx = club.findIndex((user) => user.id === playerId);
  return clubIdx >= 0 ? clubIdx + 1 : undefined;
}
...
export function resolvePublicProfile(
  ...
  const fromClub = findClubUserByIdOrNick(playerId);
