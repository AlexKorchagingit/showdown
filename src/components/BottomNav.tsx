...
export function BottomNav() {
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-50 bg-[#50444c] border-t border-[rgba(200,163,142,0.18)]"
        ...
        {NAV_ITEMS.map(({ to, label, Icon }) => (
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full bg-rg-gradient" />
                )}
                <Icon
                  size={22}
                  strokeWidth={isActive ? 2 : 1.5}
                  className={isActive ? 'drop-shadow-[0_0_6px_rgba(200,163,142,0.7)]' : ''}
                />
