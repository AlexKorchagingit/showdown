...
export function ProfileSettingsPage({ userEmail }: Props) {
  ...
  return (
    <div className="absolute inset-0 z-40 flex flex-col bg-[#110b09]">
      ...
      <div
        className="flex-1 scrollable px-5 pt-20 pb-8"
        style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 2rem)' }}
      >
        <h1 className="text-[17px] font-800 tracking-[0.2em] text-white uppercase mb-8">
          Настройки
        </h1>

        <div className="space-y-5">
          <section>
            <label className="block text-[11px] font-700 uppercase tracking-[0.18em] mb-2" style={{ color: '#D99962' }}>
              Никнейм
            </label>
            <input
              type="text"
          ...
          <section>
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-[#231A16] text-white border border-[#D99962]/30 rounded-xl px-4 py-3 outline-none focus:border-[#D99962]/60 transition-colors resize-none"
          ...
          <button
            type="button"
            onClick={handleSave}
            className="w-full h-14 rounded-2xl font-700 text-[15px] text-[#0A0908] active:scale-[0.98] transition-transform"
            style={{
              background: 'linear-gradient(to right, #8C4C27, #D99962)',
              boxShadow: '0 0 20px rgba(217,153,98,0.28)',
            }}
          >
            Сохранить
          </button>
